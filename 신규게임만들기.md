...
<script>
    ...
    async function addGamePoints(points, gameUrl) {
        try {
            const { data: { user } } = await _supabaseClient.auth.getUser();
            if (!user) {
                console.log('Not logged in, skipping point update');
                return 0;
            }

            const result = await _supabaseClient.rpc('update_user_points', {
                p_user_id: user.id,
            ...
            await _supabaseClient.rpc('increment_game_view_logged', {
                p_game_url: gameUrl
            });
    ...
    async function handleGameExit(event) {
        ...
        if (gameState === 'playing' && confirm('게임을 종료하시겠습니까?\n현재 점수가 통합 포인트에 저장됩니다.')) {
            ...
            // 점수 저장
            await addGamePoints(score, '게임파일명.html');
            await updatePointsDisplay();
    ...
    async function gameOver() {
        ...
        // 점수 저장 및 표시
        const totalPoints = await addGamePoints(score, '게임파일명.html');
        await updatePointsDisplay();
